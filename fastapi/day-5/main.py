...
# ══════════════════════════════════════════════════════════════════
# ── DAY 6 — Search, Sort, Pagination (Q1–Q3, all fixed routes) ───
...
def search_products(keyword: str = Query(..., description='Word to search in product names')):
    """
    Search products whose name contains the keyword.
    Case-insensitive. Returns friendly message if nothing found.
    """
...
def sort_products(
    ...
    Rejects any sort_by value other than 'price' or 'name'.
    """
...
def search_orders(customer_name: str = Query(..., description='Customer name to search')):
    """
    Return all orders whose customer_name contains the keyword.
    Case-insensitive. Friendly message when nothing found.
    """
...
def sort_by_category():
    """
    Group products alphabetically by category (A→Z),
    then sort by price ascending within each group.
    No query params needed.
    """
    sorted_products = sorted(products, key=lambda p: (p['category'].lower(), p['price']))

    # Build a grouped view for clarity
    grouped = {}
...
def browse_products(
    ...
    """
    Combined endpoint: filter → sort → paginate.
    All params optional. Apply in that exact order.
    """
    if sort_by not in ('price', 'name'):
        return {'error': "sort_by must be 'price' or 'name'"}
    if order not in ('asc', 'desc'):
...
def paginate_orders(
    ...
    Same pagination formula as /products/page.
    """
    total       = len(orders)
